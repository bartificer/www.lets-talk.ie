---
slug: ltp25
title: Projects
date: 2015-10-21
mp3_url: https://media.lets-talk.ie/ltp/ltp25.mp3
blurb: ""
host: bart
warnings:
  metadata:
    - unreviewed
    - unchecked_guests
  blurb:
    - no_blurb
  notes:
    - crude_import

---
<h4>Panel:</h4>
<ul>
	<li><a title="Bart Busschots" href="http://www.bartb.ie/" target="_blank">Bart Busschots</a> (host) - <a title="Bart's Twitter" href="http://twitter.com/bbusschots" target="_blank">@bbusschots</a> - <a href="http://www.flickr.com/photos/bbusschots/" target="_blank">Flickr</a></li>
<li><a title="Antonio Rosario" href="http://amrosario.com/" target="_blank">Antonio Rosario</a> from <a title="Switch to Manual" href="http://switchtomanual.com/" target="_blank">Switch to Manual</a> - <a title="Switch to Manual on Twitter" href="http://twitter.com/switch2manual" target="_blank">@switch2manual</a></li>
<li><a title="Gaz's Home Page" href="http://www.gazmaz.com/" target="_blank">Gazmaz</a> from the <a title="The My Mac Podcast" href="http://www.mymacpodcast.com/" target="_blank">My Mac Podcast</a> - <a title="Gaz's Twitter" href="http://twitter.com/gazmaz" target="_blank">@gazmaz</a> - <a title="Gaz's Flickr" href="http://www.flickr.com/photos/10268117@N05/" target="_blank">Flickr</a></li>
</ul>

<p>It's easy to run out of inspiration, so in this show the panel suggest some photo projects that might inspire people to get back out there shooting.</p>

<p>You'll find detailed show notes below the fold.</p>

<p>Reminder - you can submit questions for future Q & A shows at <a href="http://lets-talk.ie/photoq">http://lets-talk.ie/photoq</a></p>

<p><a href="http://www.mymac.com/podcast/ltp/LetsTalkPhotographgy-EP25-Projects.mp3" target="_blank"><img src="http://www.lets-talk.ie/resources/icon-MP3.png" alt="MP3 Download" style="border-width: 0px; display: inline-block; margin-width: 10px;" /></a><a href="http://www.mymac.com/ltp.xml" target="_blank"><img src="http://www.lets-talk.ie/resources/icon-RSS.png" alt="RSS Feed" style="border-width: 0px; display: inline-block; margin-width: 10px;" /></a><a href="https://itunes.apple.com/ie/podcast/lets-talk-photography/id725271225?mt=2" target="_blank"><img src="http://www.lets-talk.ie/resources/icon-iTunes.png" alt="iTunes" style="border-width: 0px; display: inline-block; margin-width: 10px;" /></a></p>

<p>[audio mp3="http://www.mymac.com/podcast/ltp/LetsTalkPhotographgy-EP25-Projects.mp3"]</p>

<!--more-->

<ul>
  <li><strong>Suggestion 1: Fungi (from Gaz)</strong>
    <ul>
      <li>
        <a href="https://www.flickr.com/photos/10268117@N05/sets/72157660164544625/" target="_blank">Gaz's Fungus shots</a>
      </li>
      <li>
        Bart's Fungus shots:
        <a data-flickr-embed="true"  href="https://www.flickr.com/photos/bbusschots/9130096718/" title="Scruffy Twiglet Mushrooms"><img src="https://farm8.staticflickr.com/7438/9130096718_d3459d6037.jpg" width="500" height="500" alt="Scruffy Twiglet Mushrooms"></a><script async src="//embedr.flickr.com/assets/client-code.js" charset="utf-8"></script>
        <a data-flickr-embed="true"  href="https://www.flickr.com/photos/bbusschots/9130097104/" title="Club Foot Toadstool"><img src="https://farm3.staticflickr.com/2871/9130097104_e1d465a4a5.jpg" width="500" height="333" alt="Club Foot Toadstool"></a><script async src="//embedr.flickr.com/assets/client-code.js" charset="utf-8"></script>
      </li>
    </ul>
  </li>
  <li>
    <strong>Suggestion 2: Photograph out of a single Window over time (from Antonio)</strong>
    <ul>
      <li>
        <a href="http://amrosario.photoshelter.com/gallery/The-View-From-Ocean-Parkway/G0000C038eo6U65c" target="_blank">Antonio's gallery 'The View from Ocean Parkway'</a>
      </li>
      <li>
        <a href="http://ahae.com/collections/" target="_blank">Korean Photographer Ahae who shoots all his work out of a single window</a>
      </li>
    </ul>
  </li>
  <li>
    <strong>Suggestion 3: 'four seasons' (from Bart)</strong>
    <ul>
      <li>Photography the same shot four times - once in Spring, Summer, Autumn and Winter, then assemble into a 2x2 collage.</li>
      <li>
        A half-done example (just 2 seasons):
        <a data-flickr-embed="true"  href="https://www.flickr.com/photos/bbusschots/4441352869/" title="The Changing of the Seasons"><img src="https://farm5.staticflickr.com/4003/4441352869_43108606db.jpg" width="500" height="236" alt="The Changing of the Seasons"></a><script async src="//embedr.flickr.com/assets/client-code.js" charset="utf-8"></script>
      </li>
    </ul>
  </li>
  <li>
    <strong>Suggestion 4: 'Pokemon-style project' (gotta catch 'em all)</strong>
    <ul>
      <li>Projects where you decide to try capture all of X, e.g. All 28 species of butterfly in Ireland, all types of train in use by Irish Rail etc..</li>
    </ul>
  </li>
  <li>
    <strong>Suggestion 5: 're-enact the past' (sent in by Stefaan Lesage)</strong>
    <ul>
      <li>Re-create a photo from the past today.</li>
      <li><a href="http://www.invisibleworks.co.uk/ghosts-of-the-great-war/" target="_blank">Ghosts of the Great War</a></li>
      <li><a href="https://www.flickr.com/photos/amrosario/sets/72157629833707084" target="_blank">Antonio's JFK photos</a></li>
      <li><a href="https://vimeo.com/20800502" target="_blank">A trailer for Antonio's Ghosts of Kings</a></li>
    </ul>
  </li>
  <li>
    <strong>Suggestion 6: pick an abstract word or phrase (from Antonio)</strong>
  </li>
  <li><a href="http://www.malpastowers.com/Pictures/">Gaz's Walk project</a></li>
  <li>
    Links suggested by Gaz:
    <ul>
      <li><a href="http://www.digitalcameraworld.com/2015/01/06/52-photography-projects-a-photo-idea-try-every-week-2015/" target="_blank">52 photography projects: a photo idea to try every week in 2015</a></li>
      <li><a href="http://improvephotography.com/11546/21-best-today-photo-ideas/" target="_blank">The 21 Best Photography Project Ideas I’ve Ever Seen</a></li>
      <li><a href="http://www.photoventure.com/2014/11/05/6-great-photo-projects-you-can-do-without-leaving-your-home/" target="_blank">6 great photo projects you can do without leaving your home</a></li>
    </ul>
  </li>
</ul>